<script>
    ...
    export let prefix_matching_scores;
    export let copying_scores;
    export let induction_scores;
    export let show_tokens = true;
    // export let blank_color = "#FFF";
    prefix_matching_scores = prefix_matching_scores.map((score) =>
        Number(score.toPrecision(3))
    );
    copying_scores = copying_scores.map((score) =>
        Number(score.toPrecision(3))
    );
    induction_scores = induction_scores.map((score) =>
        Number(score.toPrecision(3))
    );
    ...
    $: attention_reduce_dst = show_info_weighted
        ? attention_reduce_dst_info_weighted
        : attention_reduce_dst_unmodified;
    $: attention_reduce_src = show_info_weighted
        ? attention_reduce_src_info_weighted
    ...
    function head_intensity(head_i, focus_token_value, hover_token_is_target) {
        if (focus_token_value == undefined) {
            var v = 1.0;
        } else {
            var reduced_attn = hover_token_is_target
                ? attention_reduce_src
                : attention_reduce_dst;
            var v = Math.max(
                0,
                Math.min(1, reduced_attn[focus_token_value][head_i])
            );
        }
        return "" + v;
    }
    ...
    $: all_token_colors = range(tokens.length).map((i) =>
        token_color(
            attention_show,
            focus_token,
            i,
...
<div class="attn-container">
    ...
    {#if info_weighted !== undefined}
        <div
            style="grid-column: big-attn; grid-row: main; {show_info_weighted
                ? ''
                : 'display:none;'}"
        >
            <ArrayImage
    ...
    {#if N_heads > 1}
        ...
        <div class="heads">
            {#each range(attention_show.shape[2]) as head_i, index}
                <LockableValueToggle
                ...
                >
                    ...
                    <p>Induction score</p>
                    <p>{induction_scores[index]}</p>
                    <br />
                    <p>Prefix-matching score</p>
                    <p>{prefix_matching_scores[index]}</p>
                    <br />
                    <p>Copying score</p>
                    <p>{copying_scores[index]}</p>
                </LockableValueToggle>
            {/each}
        </div>
    {/if}
        ...
        <div class="toggle">
            <nobr>
                <input
                    class="hover-mode"
                    type="checkbox"
                    bind:checked={hover_token_is_target}
                />
                <span
                    class="hover-mode-text"
                    style="white-space: nowrap;"
                    on:click={() =>
                        (hover_token_is_target = hover_token_is_target ^ true)}
                >
                    Selected is
                ...
                {#if info_weighted !== undefined}
                    <input
                        class="info-mode"
                        type="checkbox"
                        bind:checked={_show_info_weighted}
                    />
                    <span
                        class="info-mode-text"
                        style="white-space: nowrap;"
                        on:click={() =>
                            (_show_info_weighted = _show_info_weighted ^ true)}
                    >
                        Attention is
                        <b
                            >{_show_info_weighted
...
<style>
    ...
    .tokens-container {
        ...
        grid-template-columns:
            [left] min-content [right] minmax(min-content, 800px)
            [end];
        gap: 12px;
        margin-top: 24px;
        color: white;
